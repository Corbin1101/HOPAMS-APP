...
class Doctors(Resource):
    ...
    def post(self):
        ...
        doc_specialization = doctorInput['doc_specialization']
        doctorInput['doc_id']=conn.execute('''INSERT INTO doctor(doc_first_name,doc_last_name,doc_ph_no,doc_specialization)
            VALUES(?,?,?,?)''', (doc_first_name, doc_last_name,doc_ph_no,doc_specialization)).lastrowid
        conn.commit()
        return doctorInput
...
class Doctor(Resource):
    ...
    def put(self,id):
        ...
        doc_specialization = doctorInput['doc_specialization']
        conn.execute(
            "UPDATE doctor SET doc_first_name=?,doc_last_name=?,doc_ph_no=?,doc_specialization=? WHERE doc_id=?",
            (doc_first_name, doc_last_name, doc_ph_no, doc_specialization, id))
        conn.commit()
        return doctorInput